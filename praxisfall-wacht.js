/*
  ...
  MSV-Praxisfall-Wacht_Master_v03_2026-08-23-Web

  Diese öffentliche Google-Sheets-Datei wird automatisch
  aus der internen Webansicht des privaten Masters gespeist.

  Der private Master bleibt die Single Source of Truth.
*/
...
function loadPublishedSheet() {
  ...
  if (
    ...
    showLoadError(
      "Die Google-Charts-Bibliothek konnte nicht geladen werden."
    );
    return;
  }
  ...
  google.charts.setOnLoadCallback(() => {
    try {
      const query = new google.visualization.Query(GVIZ_URL);
...
function dataTableToCases(table) {
  ...
  for (let c = 0; c < table.getNumberOfColumns(); c++) {
    labels.push(
      String(table.getColumnLabel(c) || "").trim()
    );
  }
